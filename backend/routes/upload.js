...
// 3. POST upload completed apprentice list (validate or sync)
router.post('/completed', authMiddleware, (req, res, next) => {
  const isJson = req.headers['content-type'] && req.headers['content-type'].includes('application/json');
  if (isJson) {
    return next();
  }
  ...
  if (user.role !== 'Super HR') {
    return res.status(403).json({ success: false, error: 'Permission denied. Only Super HR can upload completed apprentices.' });
  }
  ...
  try {
    ...
    if (dryRun) {
      const report = await sheetsService.validateCompletedApprentices(parsedRecords);
      const store = requestStorage.getStore();
      if (store) {
        store.recordCount = parsedRecords.length;
      }
      return res.json({
        ...
        totalProcessed: report.totalProcessed,
        matchedActive: report.matchedActive,
        alreadyCompleted: report.alreadyCompleted,
        newCompleted: report.newCompleted,
        duplicates: report.duplicates,
        invalid: report.invalid,
        rejected: report.rejected
      });
    } else {
      const { completionReason, otherCompletionReason, completionRemarks, overwriteCompletion } = req.body;
      if (!completionReason) {
      ...
      const result = await sheetsService.syncCompletedApprentices({
        records: parsedRecords,
        fileName: originalName,
        completionReason,
        otherCompletionReason,
        completionRemarks,
        overwriteCompletion: overwriteCompletion === true || overwriteCompletion === 'true',
        updatedBy,
        user
      });
    ...
    console.error('Completed upload route error:', err);
    const sanitizedError = err.message.replace(/([a-zA-Z]:\\[\w\\\.-]+)/g, '[Internal Path]');
    return res.status(500).json({ success: false, error: 'Internal server error: ' + sanitizedError });
  }
});
